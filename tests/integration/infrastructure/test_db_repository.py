...
from pyzn.domain.model import Downloads, ProjectVersionDownloads, ProjectName, Project, DayDownloads
...
def mongo_client():
    ...
    client.pyzn_test.projects.delete_many({})
    client.pyzn_test.project_downloads.delete_many({})
...
def test_save_project_with_new_format(mongo_client: MongoClient, repository: ProjectRepository):
    ...
    data = mongo_client.pyzn_test.projects.find_one({"name": project.name.name})
    expected_data = {"name": "climoji", "total_downloads": 190, "monthly_downloads": 0}
    for key, value in expected_data.items():
        assert key in data
        assert value == data[key]
    downloads_data = sorted(
        mongo_client.pyzn_test.project_downloads.find({"project": project.name.name}), key=lambda x: x["date"]
    )
    expected_downloads_data = [
        {
            ...
            "downloads": [
                ...
                {"version": "2.0.1", "downloads": 30, "pip_downloads": 30},
            ],
        },
        {
            "project": "climoji",
            "date": "2020-04-01",
            "downloads": [{"version": "2.0", "downloads": 20, "pip_downloads": 20}],
        },
    ]
    ...
    expected_downloads_data = [
        {
            ...
            "date": "2020-03-31",
            "downloads": [
                {"version": "2.0", "downloads": 40, "pip_downloads": 10},
                {"version": "2.0.1", "downloads": 30, "pip_downloads": 10},
            ],
        },
        {
            "project": "climoji",
            "date": "2020-04-01",
            "downloads": [{"version": "2.0", "downloads": 20, "pip_downloads": 10}],
        },
    ]
    assert len(expected_downloads_data) == len(downloads_data)
    for i in range(len(expected_downloads_data)):
...
def test_retrieve_project_with_new_format(mongo_client: MongoClient, repository: ProjectRepository):
    ...
    query = {"name": "climoji"}
    mongo_client.pyzn_test.projects.replace_one(query, data, upsert=True)
    downloads_data = [
        InsertOne(
            {
                "project": "climoji",
                "date": "2020-04-01",
                "downloads": [{"version": "2.0", "downloads": 30, "pip_downloads": 30}],
            }
        ),
        InsertOne(
            {
                "project": "climoji",
                "date": "2020-04-02",
                "downloads": [{"version": "2.0", "downloads": 10, "pip_downloads": 10}],
            }
        ),
        InsertOne(
            {
                "project": "climoji",
                "date": "2020-03-31",
                "downloads": [{"version": "2.0", "downloads": 40, "pip_downloads": 40}],
            }
        ),
        InsertOne(
            {
    ...
    mongo_client.pyzn_test.project_downloads.bulk_write(downloads_data)
    ...
    expected_last_downloads = [
        ProjectVersionDownloads(datetime.date(2020, 3, 31), "2.0", Downloads(40), Downloads(40)),
        ProjectVersionDownloads(datetime.date(2020, 4, 1), "2.0", Downloads(30), Downloads(30)),
        ProjectVersionDownloads(datetime.date(2020, 4, 2), "2.0", Downloads(10), Downloads(10)),
        ProjectVersionDownloads(datetime.date(2020, 4, 3), "2.0", Downloads(30), Downloads(30)),
    ]
...
def test_retrieve_project_with_old_format(mongo_client: MongoClient, repository: ProjectRepository):
    ...
    assert Downloads(1100) == result.total_downloads
    expected_last_downloads = [
        ProjectVersionDownloads(datetime.date(2020, 3, 31), "2.0", Downloads(40), Downloads(0)),
        ProjectVersionDownloads(datetime.date(2020, 4, 1), "2.0", Downloads(30), Downloads(0)),
        ProjectVersionDownloads(datetime.date(2020, 4, 2), "2.0", Downloads(10), Downloads(0)),
        ProjectVersionDownloads(datetime.date(2020, 4, 3), "2.0", Downloads(30), Downloads(0)),
    ]
    assert expected_last_downloads == result.last_downloads()
