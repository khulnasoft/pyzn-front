...
from pyzn.application.command import UpdateVersionDownloads
from pyzn.domain.model import Password
from pyzn.domain.pypi import Row
...
@given("the {number} projects and pyzn")
def step_impl(context: Context, number: str):
    ...
    bq_rows.append(Row("pyzn", str(uuid.uuid1()), datetime.datetime.now().date(), random.randint(1, 100_000_000)))
    context.container.stats_viewer.set_data(bq_rows)
