Feature: show all data for admins
  ...
  Scenario: show all the downloads if the token is present
    ...
    And the pyzn project with the following downloads
      ...
      | 2018-05-03 | 2.0     | 25        |
      | 2018-05-04 | 2.0     | 50        |
    When I send the GET request to /api/v1/admin/projects/pyzn?password=pyznrocks
  ...
  Scenario: fail to show data if password is not present
    ...
    And the pyzn project with the following downloads
      | date       | version | downloads |
      | 2018-05-01 | 1.0     | 10        |
    When I send the GET request to /api/v1/admin/projects/pyzn
  ...
  Scenario: fail to show data if password not correct
    ...
    And the pyzn project with the following downloads
      | date       | version | downloads |
      | 2018-05-01 | 1.0     | 10        |
    When I send the GET request to /api/v1/admin/projects/pyzn?password=wrongpass
    Then the response status code should be 401
    And the api response should be
    """
    {
